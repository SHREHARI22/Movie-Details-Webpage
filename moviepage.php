...
<body>
                ...
                <div class="swiper-slide slide">
                    <div class="content">
                        ...
                        while($row = $res->fetch_assoc())
                        {
                            ...
                            echo  '<h2 class="product-category"><a href="watchlist.php?id='.$_GET["id"].'&user='.$_GET["user"].'" class="btn btn-success">Add to watchlist</a></h2>';
                        }
                    ?></h3>
                        <?php
                        $sql="select * from movies where movie_name='{$_GET["id"]}'";
    ...
    <?php echo '<form action="rate.php?user='.$_GET['user'].'" method="post" >';?> 
        <h1 class="heading">Give the comments about the movie</h1>
        ...
        if($y<=10)
            {
                        ...
                        $sql1="SELECT * FROM rating WHERE movie_name='{$_GET["id"]}' and id=$i";
                        ...
                        if($res1->num_rows>0)
                        {
                            ...
                            {
                                $sql2="SELECT * FROM loginn WHERE email='{$row1["user"]}' Limit 1";
                                $res2 = $conn->query($sql2);
                                if($res2->num_rows>0)
                                {
                                    while($row2 = $res2->fetch_assoc())
                                    {
                                        echo "<h3>".$row2['Firstname']."</h3>";
                                    }
                                }
